  ...
  console.log('in get pets route');
    pool.connect(function(errConnectingToDatabase, db, done){
      if(errConnectingToDatabase) {
        ...
        db.query('SELECT owner_id, pet_id, first_name, last_name, name, breed, color, check_in_date, check_out_date FROM owners JOIN pets on pets.owner_id = owners.id JOIN visits on pets.id = visits.pet_id;'), function(errMakingQuery, result){
          done();
          if(errMakingQuery) {
